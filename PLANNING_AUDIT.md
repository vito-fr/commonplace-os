...
| Relationships as one-row graph edges | **Keep** | Symmetric types canonical-ordered, single row. Reverse edges never materialized as source of truth. | `relationships` table with CHECK constraint; `relationship_types` registry. |
...
| Semantic + visual search | **Defer to v1.5** | sqlite-vec scaffolding required now to avoid migration pain; query layer waits. | Schema accommodates vector columns and an `embeddings` table; UI does not expose semantic/visual search yet. |
...
See section 5. v1 minimum: a `rights_status` field on items with values `unknown`, `reference_only`, `licensed`, `owned`. No expiry, no license linking, no enforcement. Just enough that "is this safe to use in a campaign" has a recorded answer.
...
- `migrations/` — auto-generated by tooling, hand-reviewed before merge.
...
- **Examples:** `rights_status` (unknown / reference_only / licensed / owned), `rights_note` (free-text origin: "Pinterest, public board, photographer credit unknown").
- **Created by:** Owner. Imports default to `unknown`.
- **v1 required:** Field exists; population is encouraged but not enforced.
- **AI suggest:** No (legal status is not AI's call).
- **AI write direct:** No.
- **Schema location:** `items.rights_status`, `items.rights_note`.
...
- **Full-text search** over canonical title + description + summary + tag names — SQLite FTS5 virtual table.
...
1. **An `embeddings` table** with `(item_id, field_name, model_name, model_version, vector)` — empty in v1, populated when semantic search ships. Use sqlite-vec extension.
...
3. **A canonical `searchable_text` derivation** — define which fields feed the FTS5 index. Locking this now prevents index rebuilds later.
4. **Vector storage layout decision** — sqlite-vec native columns vs. binary blobs. Decide before writing the migration.
...
- **What is backed up:** the full PocketBase SQLite database, all uploaded files (PocketBase-managed), and the contents of `migrations/` and `docs/` (via the git repository, separate concern).
...
**Migrations:**
5. Creating any file in `migrations/`
...
6. Vector storage layout in sqlite-vec (native columns vs. binary blobs)
